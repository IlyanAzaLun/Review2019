...
<div class="row col-lg-12">
		...
		<table class="table table-bordered table-hover table-striped" id="dataBarang">
			...
			<tbody>
				<tr>
					...
					<td align="center">
						<a href="" data-toggle="modal" data-target="#viewGambar" data-nama="<?=$data->nama_brg?>" data-detail="<?=$data->detail_brg?>" data-gambar="<?=$data->gbr_brg?>" id="picture">
							<img src="<?='assets/img/barang/'.$data->gbr_brg;?>" alt="" width="35px">
						</a>
					</td>
					<td align="center">
						<a href="" id="edit_brg" data-toggle="modal" data-target="#edit" data-id="<?=$data->id_brg?>" data-nama="<?=$data->nama_brg?>" data-harga="<?=$data->harga_brg?>" data-stok="<?=$data->stok_brg?>" data-detail="<?=$data->detail_brg?>" data-gambar="<?=$data->gbr_brg?>">
			...
			<div class="modal-content">
				...
				<form action="" method="post" enctype="multipart/form-data">
					<div class="modal-body">
						...
						<div class="form-group">
							<label for="detail_brg" class="control-label">Detail Barang</label>
							<textarea type="text" class="form-control" id="detail_brg" name="detail_brg" required></textarea>
						</div>
						<div class="form-group">
				...
				<?
					if (@$_POST['tambah']) {
						...
						$cek = $brg->cekbarang($nama_brg);
						if ($cek->num_rows > 0) {
							echo "<script>alert('Data Barang dengan nama : ".$nama_brg." Sudah ada!');</script>";
						}else{
							...
							if ($upload) {
								$brg->tambah($nama_brg, $harga_brg, $stok_brg, $detail_brg, $gbr_brg);
	...
	<script>
		$(document).on("click", "#edit_brg" ,function(){
			...
			var detail_brg = $(this).data('detail');
			...
			$("#modal-edit #detail_brg").val(detail_brg);
		...
		$(document).on("click", "#picture", function(){
			...
			$("#viewGambar .detail_brg").html(detail_brg);
